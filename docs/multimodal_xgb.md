# Multimodal Fusion (Merge + Train XGBoost)

This workflow fuses the four modality-specific processed feature tables into a
single multimodal dataset and trains one XGBoost model with GPU acceleration on the merged 64-feature
matrix.
...
## Step 2: Train the Multimodal XGBoost Model

`train_multimodal_xgb.py` uses the standard training pipeline (scaling, Optuna tuning,
plots, metrics, SHAP, and reports) to train an XGBoost model on the 64-feature merged dataset.
It natively uses GPU acceleration (`tree_method="gpu_hist"`) and tree-based SHAP analysis.
...
python src/train_multimodal_xgb.py `
