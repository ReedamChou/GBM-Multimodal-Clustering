#!/usr/bin/env python3
"""Multimodal GBM risk classifier training workflow (XGBoost GPU)."""
...
_XGB_DEVICE_PARAMS: dict[str, str] | None = None


def get_xgb_device_params() -> dict[str, str]:
    global _XGB_DEVICE_PARAMS
    if _XGB_DEVICE_PARAMS is not None:
        return _XGB_DEVICE_PARAMS
    try:
        clf = XGBClassifier(device="cuda", tree_method="hist", n_estimators=1)
        clf.fit(np.array([[1, 2], [3, 4]]), np.array([0, 1]))
        _XGB_DEVICE_PARAMS = {"device": "cuda", "tree_method": "hist"}
    except Exception:
        try:
            clf = XGBClassifier(tree_method="gpu_hist", predictor="gpu_predictor", n_estimators=1)
            clf.fit(np.array([[1, 2], [3, 4]]), np.array([0, 1]))
...
def detect_xgb_device() -> dict[str, object]:
    params = get_xgb_device_params()
    gpu_available = "cuda" in params.get("device", "") or "gpu" in params.get("tree_method", "")
    return {
        "selected_device": "gpu" if gpu_available else "cpu",
        "gpu_available": gpu_available,
        "reason": f"XGBoost configured with {params}",
    }
...
def fit_best_model(
    ...
    random_state: int,
) -> tuple[XGBClassifier, RobustScaler]:
    scaler = RobustScaler()
    ...
    model = XGBClassifier(
        **get_xgb_device_params(),
        scale_pos_weight=scale_pos_weight(y_trainval),
        random_state=random_state,
        use_label_encoder=False,
        eval_metric="logloss",
        **best_params,
...
def save_model_artifacts(model: XGBClassifier, scaler: RobustScaler, outputs_root: Path) -> None:
    joblib.dump(
...
def main() -> None:
    ...
    device_summary = detect_xgb_device()
    ...
    X_train_scaled = pd.DataFrame(
        val_scaler.fit_transform(X_train),
        columns=X_train.columns,
        index=X_train.index,
    )
    X_val_scaled = pd.DataFrame(
        val_scaler.transform(X_val),
    ...
    best_model, scaler = fit_best_model(
        X_trainval=X_trainval,
        y_trainval=y_trainval,
